...
{% hint style="info" %}
**我今天只解决一个问题：一段人类可读的文字，如何一步步变成语言模型能够计算的数字？**
{% endhint %}
...
我现在明白，这些数字并不矛盾，因为它们统计的是不同层次的单位。
...
{% hint style="warning" %}
我不能笼统地说“一个字符就是一个 Unicode 编码”。用户感知字符、抽象字符、码点和字形不是同一个概念。
{% endhint %}
...
UTF-8、UTF-16 和 UTF-32 都是 Unicode 编码形式。这里我只关注最常见的 UTF-8。
...
从结果中我可以看到：Python 的 `len(str)` 统计码点数量，不是 UTF-8 字节数，也不保证等于用户感知字符数。
...
规范化会在 Day 002 深入学习。我现在只需要记住：**视觉相同不保证底层码点序列相同。**
...
{% hint style="warning" %}
我不能因为 Token ID 是 5000，就认为它比 ID 为 100 的 Token“更重要”或“语义更大”。ID 只是离散索引。
{% endhint %}
...
我这里使用的是 `bert-base-chinese` 当前公开词表与 Tokenizer 配置。模型文件由 Google BERT 团队发布在 Hugging Face Hub。[bert-base-chinese files](https://huggingface.co/google-bert/bert-base-chinese/tree/main)
...
我需要注意：这不是说 Unicode 无法表示它们，而是说**这个具体 Tokenizer 的词表和规则没有找到合适 Token**。
...
这就是我对“文本如何变成模型能够计算的数字”这个问题的完整回答。
