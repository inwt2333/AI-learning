...
{% hint style="info" %}
**今天只解决一个问题：模型的词表不可能收录所有文本，那么没有收录的内容如何变成 Token ID，模型又会损失什么信息？**
{% endhint %}
...
1. `自然语言处理` 在训练语料出现过，但因为频次只有 1，仍没有进入最终词表；
...
\[
\text{OOV token rate}
= \frac{\text{OOV token 实例数}}{\text{全部 token 实例数}}
\]
...
\[
\text{OOV type rate}
= \frac{\text{不同 OOV type 数}}{\text{全部不同 type 数}}
\]
